...
require(rgdal)
require(dplyr)
ltla<-st_read("EN_LTLA_FOR_EP.shp")
mob.cluster<-readRDS("mobility.temporal.cluster.rda")
...
map.dtw.h04<-ggplot(ltla)+geom_sf(aes(fill=CLU))+labs(fill="Cluster")+
  theme_bw()+
  theme(legend.position = "bottom",plot.title = element_text(size =rel(3), face = "bold"),
  ...
  geom_label_repel(data=la.coords, aes(X, Y, label = NAME),
                 colour = "Black",size=rel(4),family="Raleway",fontface="bold",
...
lasso.fit.re<-glmnet(x.update,y,family = "multinomial", alpha=1, standardise=FALSE, type.multinomial = "ungrouped", nfolds=10) #trace.it = TRUE)
plot(lasso.fit, xvar = "lambda", label = TRUE, type.coef = "2norm")
#Graphical parameters showing the ℓ2-norm in one figur
best_lambda_lasso<-min(lasso.fit$lambda)

lasso.cvfit.re <- cv.glmnet(x.update, y, family = "multinomial", alpha=1, standardise=FALSE, type.multinomial = "ungrouped", type.measure="mse",nfolds=10)
plot(lasso.cvfit.re)
...
coef.all$name<-as.factor(coef.all$name)
levels(coef.all$name)<-c("Constant","Cum. COVID-19 infection rates","Cum. COVID-19 moratlity rates", "%Bangladeshi","%Caribbean","%Other Black","%Low income HHs","%High income HHs","%Self-employed","%Social grade C1","HHs with 1 vehicle","HHs with more than 3 vehicles","%Other Asian","%Medium income HHs","%more than 3 bed rooms","Hospitals","%African","Residential population density")

require(forcats)
coef.all$name2<-fct_relevel(coef.all$name,"Constant","%Low income HHs","%Medium income HHs","%High income HHs","%Social grade C1", "%more than 3 bed rooms","Residential population density","HHs with 1 vehicle","HHs with more than 3 vehicles","Hospitals","%Self-employed","Cum. COVID-19 infection rates","Cum. COVID-19 moratlity rates","%Bangladeshi","%Other Asian","%African","%Caribbean","%Other Black")
write.csv(coef.all,"lasso.estimated.coefficients.30Mar2023.csv")
...
ggsave("lasso.mnl.std.coefficient.re.30Mar2023.png",plot.std.coefficient,width=200, height=200, units = "mm", dpi = 300, bg = "white")
...
plot.rank.features<- ggplot(data = rel.importance.trans) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        legend.position = "none") + facet_grid(~group)
plot.rank.features
ggsave("lasso.mnl.rank.features.no.scale.29Aug21.png",plot.rank.features,width=200, height=200, units = "mm", dpi = 300, bg = "white")
write.csv(rel.importance,"rel.importance.feature.selection.no.scale.30Aug2021.csv")
